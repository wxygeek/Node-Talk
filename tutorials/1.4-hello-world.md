...
3. `fs.readFile`fs模块，提供了许多文件操作方法，其中以readFile最为典型。readFile方法，asynchronously reads the entire contents of a file，即异步阅读一个文件的内容。运行到这句时，程序向fs模块发出了一个readFile指令，然后没有等待readFile方法返回，直接继续运行这条语句之后的程序。readFile方法会在后台的另外的进程中进行系统文件IO，读到内容后触发作为第二个参数出入的`回调函数`，这即为Node`异步`的实现。所以以下代码的执行结果是：
...
4. `function (err, data) { if(err) throw err; console.log(data.toString());}`
